...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log('Telnyx webhook body:', JSON.stringify(body, null, 2));
    ...
    if (!parsed) {
      console.error('Failed to parse Telnyx webhook');
      return NextResponse.json({ error: 'Invalid payload' }, { status: 400 });
    }
    
    const { from, text, messageId, receivedAt } = parsed;
    console.log('Parsed webhook:', { from, text: text?.substring(0, 50), messageId });
    ...
    console.log('=== TELNYX WEBHOOK COMPLETED ===');
    return NextResponse.json({ received: true });
...
function parseTelnyxWebhook(body: any): { from: string; to: string; text: string; messageId: string; receivedAt: Date } | null {
  try {
